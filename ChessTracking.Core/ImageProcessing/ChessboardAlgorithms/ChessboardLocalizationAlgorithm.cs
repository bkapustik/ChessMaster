...
class ChessboardLocalizationAlgorithm : IChessboardLocalizationAlgorithm
{
    public (Chessboard3DReprezentation boardReprezentation, SceneCalibrationSnapshot snapshot) LocateChessboard(ChessboardTrackingCompleteData chessboardData)
    {
        ...
        {
            BinarizationImage = ((Bitmap)binarizedImage.Convert<Rgb, byte>().AsBitmap().Clone()).HorizontalFlip(),
            CannyImage = ((Bitmap)cannyDetectorImage.Convert<Rgb, byte>().AsBitmap().Clone()).HorizontalFlip(),
            GrayImage = ((Bitmap)grayImage.Convert<Rgb, byte>().AsBitmap().Clone()).HorizontalFlip(),
            MaskedColorImage = ((Bitmap)chessboardData.PlaneData.MaskedColorImageOfTable.Convert<Rgb, byte>().AsBitmap().Clone()).HorizontalFlip()
        };
    ...
    private Image<Gray, byte> GetBinarizedImage(Image<Gray, byte> grayImage, bool otzuActive, int binaryThreshold)
    {
        ...
        if (otzuActive)
        {
            var computedOtzuThreshold = CustomOtsuBinarization.GetOtsuThreshold(grayImage.AsBitmap(), maskedValue: 255);
